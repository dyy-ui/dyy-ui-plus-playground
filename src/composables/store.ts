...
import elementPlusCode from '../template/dyy-ui-plus.js?raw'
...
export type VersionKey = 'vue' | 'elementPlus' | 'typescript' | 'dyyUiPlus'
...
const ELEMENT_PLUS_FILE = 'src/dyy-ui-plus.js'
...
export const useStore = (initial: Initial) => {
  ...
  const versions = reactive<Versions>({
    ...
    dyyUiPlus: saved?._o?.dyyUiPlusVersion ?? 'latest',
  })
  ...
  Object.assign(userOptions, {
    ...
    dyyUiPlusVersion: saved?._o?.dyyUiPlusVersion,
  })
  ...
  function generateElementPlusCode(version: string, styleSource?: string) {
    ...
    const dyyStyle =
      'https://cdn.jsdelivr.net/npm/dyy-ui-plus@latest/dist/index.css'
    return elementPlusCode
      .replace('#DYYSTYLE#', dyyStyle)
      .replace('#STYLE#', style)
  ...
  async function setVersion(key: VersionKey, version: string) {
    switch (key) {
      ...
      case 'typescript':
        ...
        break
      case 'dyyUiPlus':
        versions.dyyUiPlus = version
        userOptions.dyyUiPlusVersion = version
        break
    }
  }
